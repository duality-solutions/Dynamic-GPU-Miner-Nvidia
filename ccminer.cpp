...
static bool stratum_gen_work(struct stratum_ctx *sctx, struct work *work)
{
    ...
    // work->data[0] is version
    work->data[0] = le32dec(sctx->job.version);

    // work-data 1-8 => basically 8 32 bit chunk of the total 256 bit prevhash
    for (i = 0; i < 8; i++)
    ...
    if (opt_algo == ALGO_DECRED) {
        ...
        //work->data 9-16 -> 8 chunks of merkel root
        for (i = 0; i < 8; i++)
            work->data[9 + i] = be32dec((uint32_t *)merkle_root + i);
    ...
    if (opt_debug && opt_algo != ALGO_DECRED && opt_algo != ALGO_EQUIHASH) {
...
static void *miner_thread(void *userdata)
{
    ...
    while (!abort_flag) {
        ...
        if (opt_algo == ALGO_DECRED) nodata_check_oft = 4; // testnet ver is 0
...
void parse_arg(int key, char *arg)
{
    ...
    case 'i':
        ...
        v = (uint32_t) d;
        if (v < 0 || v > 40){
            applog(LOG_ERR, "Intensity should be between 0 and 40. Use decimals for fine tuning");
            show_usage_and_exit(1);
        }
        {
            int n = 0;
...
int main(int argc, char *argv[])
{
    ...
    printf("*** " PACKAGE_NAME " " PACKAGE_VERSION " for nVidia GPUs ***\n");
    if (!opt_quiet) {
